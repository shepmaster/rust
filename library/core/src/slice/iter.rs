...
impl<'a, T> Parts<'a, T> {
    ...
    pub fn new(slice: &'a [T], n_parts: usize) -> Self {
        assert_ne!(n_parts, 0, "number of parts must be non-zero");

        let len = slice.len();
        let short_chunk_size = len / n_parts;
        let long_chunk_size = short_chunk_size + 1;
        let chunks_needing_extra = len % n_parts;

        let midpoint = chunks_needing_extra * long_chunk_size;
        let (head, tail) = slice.split_at(midpoint);

        Self {
            head: head.chunks(long_chunk_size),
            tail: tail.chunks(cmp::max(1, short_chunk_size)),
        }
    }
    ...
    // this type doubled.
    fn chained(&mut self) -> crate::iter::Chain<&mut Chunks<'a, T>, &mut Chunks<'a, T>> {
