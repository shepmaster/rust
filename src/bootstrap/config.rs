...
impl TargetSelection {
    pub fn from_user(selection: &str) -> Self {
        let triple = crate::hackit(selection);
        let triple = INTERNER.intern_str(triple);

        let file = if triple == selection {
            None
        } else {
            Some(INTERNER.intern_str(selection))
        };
...
impl Config {
    ...
    pub fn parse(args: &[String]) -> Config {
        ...
        for target in
            config.hosts.iter().copied().chain(build.target.iter().map(|h| TargetSelection::from_user(h)))
        {
