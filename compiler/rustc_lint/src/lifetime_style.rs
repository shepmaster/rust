...
impl<'tcx> Info<'tcx> {
    ...
    fn reporting_span(&self) -> Span {
        if self.lifetime.is_syntactically_hidden() {
            ...
            self.lifetime.suggestion_position(self.source == LifetimeSource::Reference).1
        }
    }

    fn suggestion(&self, lifetime_name: &str) -> (Span, String) {
        self.lifetime.suggestion(lifetime_name, self.source == LifetimeSource::Reference)
    }
}
...
impl<'a, 'tcx> LifetimeInfoCollector<'a, 'tcx> {
    fn collect(ty: &'tcx hir::Ty<'tcx>, map: &'a mut LifetimeInfoMap<'tcx>) {
        let mut this = Self { type_span: ty.span, map, is_ref: false };
...
impl<'a, 'tcx> Visitor<'tcx> for LifetimeInfoCollector<'a, 'tcx> {
    ...
    fn visit_lifetime(&mut self, lifetime: &'tcx hir::Lifetime) {
        ...
        let syntax = if lifetime.is_syntactically_hidden() {
            Hidden
        } else if lifetime.is_syntactically_anonymous() {
            Anonymous
        } else {
            Named
        };

        let source = if self.is_ref { Reference } else { Path };

        let info = Info { type_span, syntax, lifetime, source };
    ...
    fn visit_ty(&mut self, ty: &'tcx hir::Ty<'tcx, hir::AmbigArg>) -> Self::Result {
        ...
        self.is_ref = matches!(ty.kind, hir::TyKind::Ref(..));
